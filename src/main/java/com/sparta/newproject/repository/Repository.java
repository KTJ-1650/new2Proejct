package com.sparta.newproject.repository;
...
public class Repository {
    ...
    public List<StudentResponseDto> inquiryMemo() {
        ...
        return jdbcTemplate.query(sql, (rs, rowNum) -> {
            ...
            projectName.setWrite_day(rs.getDate("write_day"));
            projectName.setModified_day(rs.getDate("modified_day"));
    ...
    public List<StudentResponseDto> modified_day(String modifieday,String name) {
        ...
        if (modifieday != null && modifieday.trim().isEmpty()) {
            modifieday = null;
        }

// SQL 쿼리 작성
        String sql = "SELECT * FROM major " +
                "WHERE (DATE(modified_day) = ? OR ? IS NULL) " +
                "AND (charge_name = ? OR ? IS NULL) " +
                "ORDER BY modified_day DESC";

// 쿼리 실행
        return jdbcTemplate.query(sql, (ResultSet rs, int rowNum) -> {
            // ResultSet에서 Schedule 객체로 매핑
            StudentResponseDto schedule = new StudentResponseDto();
            schedule.setStudent_id(rs.getInt("student_id"));
            schedule.setTo_do(rs.getString("to_do"));
            schedule.setCharge_name(rs.getString("charge_name"));
            schedule.setWrite_day(rs.getDate("write_day"));
            schedule.setModified_day(rs.getDate("modified_day"));
            return schedule;
        }, modifieday, modifieday, name, name);
    }
    ...
    public List<StudentResponseDto> chargename(StudentRequestDto requestDto) {
        ...
        return jdbcTemplate.query(sql,new Object[]{requestDto.getModified_day(),}, (rs, rowNum) -> {
            ...
            modify.setWrite_day(rs.getDate("write_day"));
            modify.setModified_day(rs.getDate("modified_day"));
    ...
    public List<StudentResponseDto> modicharge(StudentRequestDto requestDto) {
        ...
        return jdbcTemplate.query(sql,new Object[]{requestDto.getModified_day(),requestDto.getCharge_name()}, (rs, rowNum) -> {
            ...
            modicharge.setWrite_day(rs.getDate("write_day"));
            modicharge.setModified_day(rs.getDate("modified_day"));
